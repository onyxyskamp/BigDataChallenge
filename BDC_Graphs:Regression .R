...
#
#
# here is the data that we are using to run the model. 
...
# here is code that needed to be run in order to store the in the correct folder
working.dir <- getwd() 
output.folders <- c("1.Raw.Data","2.Clean.Data","3.Analysis","4.Graphs")
gr.path <- paste(working.dir,"/",output.folders[4], "/", sep="")

#Here are our variables 
expl.variable <- c(2,3,4,5) #The position of each explanatory column (elevation=2 etc.)
var.names <- colnames(bis)[expl.variable] #here is where the names are assigned to the postion 
...
for(i in 1:4){ #for loop 1 through 4 because we have 4 postions 
  ...
  model.t <- lm(bis$mass ~ bis[, expl.variable[i]] ) # the coefficients for each individual regression result 
